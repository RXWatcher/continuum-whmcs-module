  ...
  {else}
    <div style="border:1px solid #e5e7eb; border-radius:10px; overflow:hidden;">
      ...
      <div style="display:grid; grid-template-columns:repeat(auto-fit,minmax(150px,1fr)); gap:0.75rem 1.5rem; padding:1.25rem;">
        ...
        {if $profile_limit !== null}<div><div style="font-size:0.75rem; color:#6b7280; text-transform:uppercase; letter-spacing:0.03em;">Profiles</div><div style="font-weight:600;">{if $profiles_used !== null}{$profiles_used|intval} / {/if}{$profile_limit|intval}</div></div>{/if}
